...
function reportDeltaTime(line = 0){
	var thisDate = new Date();
	var thisTime = Number(thisDate.getTime()).toFixed(0);
	if(TIMING_DEBUG){
		if(line > 0){
			console.log("Delta time is " + (thisTime - previousTime) + " on line " + line + " from previous line " + previousLine);
		} else {
			console.log("Delta time is " + (thisTime - previousTime) + " from previous line " + previousLine);
		}
	}
...
function findPos(index, delim, quality, size, defaultSize, isInline, prevFailedStartElemIfIsEmpty = null){
	...
	if(placeHolderEnd - placeHolderStart == 2.0) { // empty equation
		...
		return [defaultSize, startElement]; // default behavior of placeImage
	}
	...
	for (; worked <6; ++worked){//[3,"https://latex.codecogs.com/png.latex?","http://www.codecogs.com/eqnedit.php?latex=","%5Cinline%20", "", "Codecogs"]
		try {
			...
			// needed for codecogs to generate equation properly, need to figure out which other renderers need this. to test, use align* equations.
 			...
 			reportDeltaTime(441);
			let didTimeOut = true;
 		// 	let FETCH_TIMEOUT = 30000; // 30 second timeout
			// const timeout = setTimeout(function() {
   //      if(didTimeOut){
			// 		throw new Error('Request timed out with ' + renderer[1]);
   //      }
   //  	}, FETCH_TIMEOUT);
