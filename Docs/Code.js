...
function retrieveSize(size, defaultSize, paragraph, childIndex, start) {
  //GET SIZE
  let newSize = size;
  if (size == 0) {
    try {
...
function getBodyFromIndex(index) {
  ...
  if (
    type === DocumentApp.ElementType.BODY_SECTION ||
...
function placeImage(index, startElement, start, end, quality, size, defaultSize, delim, isInline) {
  ...
  size = retrieveSize(size, defaultSize, paragraph, childIndex, start);
...
function getRenderer(worked) {
  ...
  if (worked == codeCogsPriority) {
    ...
    return [
      texRenderPriority,
      "http://texrendr.com/cgi-bin/mimetex?%5CHuge%20EQUATION",
      "http://www.texrendr.com/?eqn=",
      "%5Ctextstyle%20",
      "",
      "Texrendr",
      "",
    ...
    return [
      10,
      "https://texrendr.com/cgi-bin/mathtex.cgi?%5Cdpi%7B1800%7DEQUATION",
      "https://www.texrendr.com/?eqn=",
      "%5Ctextstyle%20",
      "",
      "Texrendr",
      "",
