 ...
 function replaceEquations(sizeRaw, delimiter){
	...
	for (var index = 0; index < childCount; index++){
		...
		while(true){
			...
			if(gotSize == -100000)   			// means all renderers didn't return/bugged out.
...
function getCustomEncode(equation, direction, time){
	...
	var toFind    = [["#", "+", "%0D", "%0D", "%0D"], ["\‘", "\’", "”", "“", "−", "≥", "≤", "‐", "—"]];
	var toReplace = [["+%23", "+%2B", "%5C%5C%5C%5C%20", "%5C%5C%20", "%A0"], ["'", "'", "\"", "\"", "-", "\\geq", "\\leq", "-", "-"]];//&hash;&plus; todo ≥ with \geq
...
function getBodyFromIndex(index){
	...
	if(type === DocumentApp.ElementType.BODY_SECTION || type === DocumentApp.ElementType.HEADER_SECTION || type === DocumentApp.ElementType.FOOTER_SECTION) { // handles alternating footers etc.
		return body;
	}
	return null;
}
...
function removeAll(delimRaw) {
 	...
 	for (var index = 0; index < DocumentApp.getActiveDocument().getBody().getParent().getNumChildren(); index++){
		...
		for(i=0; i < img.length; i++) {
			...
			var failure = 1;
			var found = 0;
			for (; worked <= capableDerenderers; ++worked){// example, [3,"https://latex.codecogs.com/png.latex?","http://www.codecogs.com/eqnedit.php?latex=","%5Cinline%20", "", "Codecogs"]
...
function undoImage(delim){
	...
	if (cursor) {
		...
		if (element) {
			...
			var worked = 1;
			var failure = 1;
			for (; worked <= capableDerenderers; ++worked){//[3,"https://latex.codecogs.com/png.latex?","http://www.codecogs.com/eqnedit.php?latex=","%5Cinline%20", "", "Codecogs"]
				renderer=getRenderer(worked)[2].split("FILENAME"); //list of possibly more than one string
				for (var I = 0; I < renderer.length; ++I){
					if(origURL.indexOf(renderer[I])>-1){
						debugLog("Changing: " + origURL + " by removing " + renderer[I]);
